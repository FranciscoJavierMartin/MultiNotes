...
import {compose, withHandlers, withProps, onlyUpdateForKeys} from 'recomponse';
...
const AudioItem = ({
    ...
    const Item = (
        <TouchableHighlight onPress={() => onPress && onPress()}>
            <View style={[globalStyles.withWhiteBackground, s.container,]}>
                <View style={[globalStyles.alignedRow]}>
                    <View style={s.left}>
                        <Text numerOfLines={1} ellipsizeMode="tail" style={[s.title]}>
                            {title}
                        </Text>
                        {!isPlaying && 
                            <Text numerOfLines={1} ellipsizeMode="tail"
                    ...
                    <View style={[s.right]}>
                        ...
                        <Icon size={28} IconSet={Feather} 
                            iconName={isPlaying ? 'stop-circle' : 'play'}
                            iconStyle={s.playIcon} color="#00a086"/>
                    </View>
                </View>
            </View>
        </TouchableHighlight>
    ...
    if(removeAudio){
        res = (
            <SwipeOut right={[{text: 'Delete', onPress: () => removeAudio(id), type: 'delete'}]}>
                {Item}
            </SwipeOut>
        );
    } else {
