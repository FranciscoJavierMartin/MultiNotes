...
import {durationStr} from '../../utils/dateHelper';
...
const RecordAudioScreen = ({
    ...
    if(isDoneRecording){
        ...
        <View style={s.container}>
            <View style={s.durationContainer}>
                ...
                <Text style={s.durationText}>
                    {durationStr(durationMillis)}
                </Text>
            </View>
            <TouchableOpacity
                style={[s.recordButton, s.recordingBackground]}
                onPress={onEndRecording}>
                <Icon size={100} color={colors.audio.recording}
                    IconSet={MaterialIcons} iconName="stop"
        ...
        res = (
        <View style={s.container}>
            <TouchableOpacity style={[s.recordButton, s.startRecordButton]}
                onPress={onStartRecording}>
                <Icon size={50} color={colors.audio.startRecordingIcon}
                    IconSet={MaterialIcons} iconName="keyboard-voice"
                    iconStyle={[s.recordIcon]}/>
            </TouchableOpacity>
        </View>
        );
